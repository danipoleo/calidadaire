...
STATION_MAP: Dict[str, str] = {
    "1637": "San José","1653": "Cigefi","1735": "Cartago TEC","1762": "Santa Lucía",
    "1775": "Belén CTP","1776": "Fabio Baudrit","Z01777": "Municipalidad de Santa Ana",
}
...
FILENAME_REGEX = re.compile(r"^device_(?P<sid>[A-Za-z0-9]+)_(?P<start>\d{12})_(?P<end>\d{12})_1hr\.csv$")
...
def parse_from_filename(name: str):
    ...
    sid, start, end = m.group("sid"), m.group("start"), m.group("end")
    try:
...
def _list_with_html(owner:str, repo:str, path:str, branch:str):
    # Acepta https://github.com/<o>/<r>/tree/<b>/<path> y también la raíz (path vacío)
    ...
    try:
        ...
        if rh.status_code >= 400:
            return []
        html = rh.text
        # path puede ser '', 'datos', 'datos/sub', etc.
        path_prefix = (path.strip("/") + "/") if path.strip("/") else ""
        # Regex robusto: .csv o .CSV y permite querystrings (?plain=1)
        pattern = re.compile(
            rf'href="/{re.escape(owner)}/{re.escape(repo)}/blob/{re.escape(branch)}/'
            rf'({re.escape(path_prefix)}[^"<>?]{{1,}}?\.(?i:csv))(?:\?[^"<>]*)?"'
        )
...
def github_list_files(owner:str, repo:str, path:str, branch:str, force_html:bool=True):
    # Por defecto TRUE para evitar 403 y asegurar que lista sin token.
...
def unit_from_column(df:pd.DataFrame, unit_col:Optional[str], default:str)->str:
    if unit_col and unit_col in df.columns:
        ...
        if len(val): return val.iloc[0].strip()
    return default
...
def make_line_figure(x,y,title,color,y_title,icca_ranges=None,show_icca=True):
    ...
    fig.update_layout(title=title,template="plotly_white",margin=dict(l=20,r=20,t=60,b=30),
                      xaxis_title="Fecha/Hora",yaxis_title=y_title,height=350,legend_title_text=None)
...
st.caption("Carga desde carpeta pública del repo y visualización 2×2 con bandas ICCA.")
...
with st.sidebar:
    ...
    gh_url = st.text_input("URL (p. ej. https://github.com/owner/repo/tree/branch/carpeta)", value="")
    if gh_url.strip():
        try:
...
# ---- Listar CSV (sin depender de la API) ----
...
if not files:
    st.error("No encontré CSV en esa carpeta del repositorio. Verifica owner/repo/ruta/branch.\n"
             "Sugerencia: usa una URL tipo: https://github.com/danipoleo/calidadaire/tree/main/datos")
    st.stop()
...
if not valid_years:
    st.error("No pude resolver año/mes de los archivos. ¿Los nombres siguen el patrón device_<id>_<YYYYMMDDHHMM>_... ?")
    st.stop()
...
with st.expander("Estadísticas del mes (valores originales)"):
    ...
    st.dataframe(df[cols_for_stats].describe().T.rename(
