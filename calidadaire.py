...
# --------- Listado robusto de archivos en GitHub ---------
...
def _extract_csv_links(html: str)->List[str]:
    # Captura cualquier href a blob/.../*.csv (case-insensitive)
    hrefs = re.findall(r'href="([^"]+?/blob/[^"]+?\.csv[^"]*)"', html, flags=re.IGNORECASE)
    return hrefs
...
def _list_with_html_plain(owner: str, repo: str, path: str, branch: str):
    ...
    try:
        ...
        # filtrar por que contengan la subcarpeta (substring)
        sub = path.strip("/")
        hrefs = [h for h in hrefs if (not sub) or (f"/{sub}/" in h) or h.endswith("/"+sub)]
        files = []
        seen = set()
        for h in hrefs:
            # h es como /owner/repo/blob/<algo>/datos/archivo.csv?plain=1
            rel = h.split("/blob/",1)[-1]  # <algo>/datos/archivo.csv?...
            # quitar query
            rel = rel.split("?",1)[0]
            name = rel.split("/")[-1]
...
with st.sidebar:
    ...
    st.caption("También puedes pegar la URL de carpeta:")
    gh_url = st.text_input("URL carpeta", value="", placeholder="https://github.com/owner/repo/tree/branch/carpeta")
    if gh_url.strip():
        try:
...
with st.expander("Diagnóstico de listado"):
    ...
    st.write("Resultados por estrategia:", debug[:3] + debug[3:])  # mostrar todos

if not files:
    if direct_csv_url.strip():
        # Construir raw desde un blob si es necesario
        ...
        st.error(
            ...
            "Si el problema persiste, pega la **URL directa** a un CSV en el campo 'Plan B'."
        )
        st.stop()

# Índice e intento de parsear año/mes
rows = []
for f in files:
    fname = f.get("name") or ""
    if not fname: 
    ...
    rows.append({
        "name": fname, "url": url, "sid": sid, "station": station,
...
if files_df.empty:
    st.error("No se pudo construir el índice de archivos.")
    st.stop()
...
if not valid_years:
    st.error("No pude resolver año/mes de los archivos. Verifica el patrón de nombres o usa el Plan B con un CSV puntual.")
    st.stop()
...
with st.expander("Estadísticas del
